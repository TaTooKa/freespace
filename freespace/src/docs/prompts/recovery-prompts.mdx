import Stat from "../../@rocketseat/gatsby-theme-docs/components/Stat.js"
import AffectingTraits from "../../@rocketseat/gatsby-theme-docs/components/AffectingTraits.js"
import CharacterState from '../../@rocketseat/gatsby-theme-docs/components/CharacterState.js'

<CharacterState />

# RECOVERY PROMPTS

## HEAL
⋗ **When you receive medical care or provide treatment**, envision the situation. If you...
- ...receive treatment from someone:<br/>
 test your <Stat name="PHYSIQUE"/>.
- ...mend your own wounds:<br/>
 test your <Stat name="PHYSIQUE"/> or <Stat name="INTELLECT"/>, whichever is lower.
- ...obtain treatment for a companion:<br/>
 test your <Stat name="RESOLVE"/>.
- ...provide care:<br/>
 test your <Stat name="INTELLECT"/>.

<AffectingTraits promptName="Heal"/>
<hr class="gradient-hr-turquoise"/>

&nbsp;<span class="turquoise bold">⋙ SUCCESS</span>: The care is helpful. The target of the treatment recovers **1 HEALTH**, <span class="turquoise">+X</span> if you [SACRIFICE RESOURCES (X)](/prompts/suffer-prompts#sacrifice-resources). 

> If <span class="turquoise bold">CRITICAL</span>, also choose one:<br/>
‣ The target of the treatment is now _IN CONTROL_.<br/>
‣ The target of the treatment recovers an extra **+1 HEALTH**.

<hr class="gradient-hr-fuchsia"/>

&nbsp;<span class="fuchsia bold">⋙ FAILURE</span>: The aid is ineffective and the situation worsens. You are *IN A BAD SPOT* and must [PAY THE CONSEQUENCES](/prompts/fate-prompts#pay-the-consequences).
<hr/>
<br/>

## RELAX
⋗ **When you try to unwind and relax in order to soothe your mind and spirit**, envision how you do so. If you... 
- try to forget by intoxicating your body:<br/>
 test your <Stat name="PHYSIQUE"/>.
- take some time for introspection and deep thought:<br/>
 test your <Stat name="INTELLECT"/>.
- rely on others to entertain you or share intimacy:<br/>
 test your <Stat name="RESOLVE"/>.

<AffectingTraits promptName="Relax"/>
<hr class="gradient-hr-turquoise"/>

&nbsp;<span class="turquoise bold">⋙ SUCCESS</span>: You find comfort. Recover **1 PSYCHE**, <span class="turquoise">+X</span> if you [SACRIFICE RESOURCES (X)](/prompts/suffer-prompts#sacrifice-resources). 

> If <span class="turquoise bold">CRITICAL</span>, also choose one:<br/>
‣ You are now _IN CONTROL_.<br/>
‣ You recover an extra **+1 PSYCHE**.

<hr class="gradient-hr-fuchsia"/>

&nbsp;<span class="fuchsia bold">⋙ FAILURE</span>: You take no comfort and the situation worsens. You are *IN A BAD SPOT* and must [PAY THE CONSEQUENCES](/prompts/fate-prompts#pay-the-consequences).
<hr/>
<br/>

## RESUPPLY
⋗ **When you attempt to bolster your readiness**, envision the opportunity and your approach. If you...
- barter or make an appeal: test your <Stat name="RESOLVE"/>.
- threaten or seize: test your <Stat name="PHYSIQUE"/>.
- steal or swindle: test your <Stat name="SUBTERFUGE"/>.
- scavenge or craft: test your <Stat name="INTELLECT"/>.

<AffectingTraits promptName="Resupply"/>
<hr class="gradient-hr-turquoise"/>

&nbsp;<span class="turquoise bold">⋙ SUCCESS</span>: You acquire supplies. Recover **2 RESOURCES**. 

> If <span class="turquoise bold">CRITICAL</span>, also choose one:<br/>
‣ You are now _IN CONTROL_.<br/>
‣ You recover an extra **+2 RESOURCES**.

<hr class="gradient-hr-fuchsia"/>

&nbsp;<span class="fuchsia bold">⋙ FAILURE</span>: You don’t find what you need; or you must first deal with a cost, complication, or demand. Envision the nature of this obstacle and [PAY THE CONSEQUENCES](/prompts/fate-prompts#pay-the-consequences).
<hr/>
<br/>

<div style="height: 400px;"></div>